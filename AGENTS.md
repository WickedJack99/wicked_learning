## Wicked Learning project context

Before product-affecting work, read `documentation/product.md`. For current
behavior, use `documentation/features.md` and the code/tests. For technical
structure, use `documentation/architecture.md`. Use `documentation/roadmap.md`
for active priorities. Do not resurrect historical ideas from archived
conversations as if they were current requirements.

### Change decision tree

1. Is this a current bug or regression? Reproduce it first, locate the owning
   route/action/service/component, and add a focused regression check before
   refactoring surrounding behavior.
2. Is the behavior already implemented? Reuse the existing model, action,
   query, serializer, hook or UI primitive. Preserve working upload, download,
   selection, navigation and persistence paths unless the request explicitly
   replaces them.
