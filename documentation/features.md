...
specified, the desk opens on its first available area, Connections; an explicit
area in the URL still opens that chosen section. An entirely empty desk keeps
Connections as its orientation prompt. Route and revisit
...
learner identities.
With the default one-contribution policy, playback replaces the composer after
the learner's contribution is accepted; activities explicitly configured for
repeated contributions keep the composer available.
...
reflection-created pages, server-paginated search results, writing/rendered modes,
autosaved drafts and export. The shared Journal action opens it as a focused
workspace. Learners can
...
message transcript itself remains a bounded reading area. Organization chat
shows recent messages in server-paginated pages, while the organization
directory also returns a bounded server page, so browsing a larger community
does not load every organization or message history at once.
...
node positions spatially, including each visible node's current position and
whether restoring it would move that node, before restoring it, so the history cannot be
mistaken for a complete map backup.
...
the template's activity type, title, context-sensitive references and reusable
media availability before explicitly applying it as a new editable draft.
...
while the author retains rename, sharing, deletion and update control. An
